...
if (isset($_POST["valider"])) {
    ...
    if ($tab > 0) {
        ...
        if (is_numeric($tel)) {
            $req = $connexion->prepare("INSERT INTO `membre`(`nom`, `postnom`, `genre`, `tel`, `adresse`, `etatcivil`, `matricule`, 
         `coorporation`, `lieunaissance`, `datenaissance`, `photo`, `date`,supprimer) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $req->execute(array($nom, $postnom, $genre, $tel, $adresse, $etatcivil, $matricule, $coorporation, $lieunaissance, $datenaissance, $photo, $date, $supprimer));
            if ($req) {
                $save = $connexion->prepare("INSERT INTO `beneficiaire`(`nom`, `postnom`, `genre`, `tel`, `adresse`, `etatcivil`, `matricule`,`lien`, `lieunaissance`, `datenaissance`, 
            `photo`,titulaire, `date`,supprimer) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                $save->execute(array($nom, $postnom, $genre, $tel, $adresse, $etatcivil, $matben, $lien, $lieunaissance, $datenaissance, $photo, $titulaire, $date, $supprimer));
